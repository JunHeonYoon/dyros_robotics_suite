#include "xls/controller.h"

namespace XLS
{
    XLSController::XLSController(const rclcpp::Node::SharedPtr& node, double dt, JointDict jd)
    ...
    {                   
        ...
        target_vel_sub_ = node_->create_subscription<geometry_msgs::msg::Twist>("xls_controller/cmd_vel", 10,std::bind(&XLSController::subtargetVelCallback, this, std::placeholders::_1));
        joint_sub_ = node_->create_subscription<sensor_msgs::msg::JointState>("/joint_states_raw", 10, std::bind(&XLSController::subJointStatesCallback, this, std::placeholders::_1));
        
        base_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("xls_controller/base_pose", 10);
        base_vel_pub_ = node_->create_publisher<geometry_msgs::msg::Twist>("xls_controller/base_vel", 10);
    ...
    void XLSController::starting()
    {
        base_pose_pub_timer_ = node_->create_wall_timer(std::chrono::milliseconds(100), std::bind(&XLSController::pubBasePoseCallback, this));
        base_vel_pub_timer_ = node_->create_wall_timer(std::chrono::milliseconds(100), std::bind(&XLSController::pubBaseVelCallback, this));
    }

    void XLSController::updateState(const VecMap& pos_dict, 
                                      ...
                                      double current_time)
    {
        ...
        wheel_pos_(2) = pos_dict.at("rear_left_wheel")(0);
        wheel_pos_(3) = pos_dict.at("rear_right_wheel")(0);

        wheel_vel_(0) = vel_dict.at("front_left_wheel")(0);
        wheel_vel_(1) = vel_dict.at("front_right_wheel")(0);
        wheel_vel_(2) = vel_dict.at("rear_left_wheel")(0);
        wheel_vel_(3) = vel_dict.at("rear_right_wheel")(0);
    ...
    CtrlInputMap XLSController::getCtrlInput() const
    {
     ...
     void XLSController::setMode(const std::string& mode)
    {
    ...
    void XLSController::keyCallback(const std_msgs::msg::Int32::SharedPtr msg)
    {
    ...
    void XLSController::subtargetPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
    ...
    void XLSController::subtargetVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
    {
        RCLCPP_INFO(node_->get_logger(),
    ...
    void XLSController::subJointStatesCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
    {
    ...
    void XLSController::pubBasePoseCallback()
    {
    ...
    void XLSController::pubBaseVelCallback()
    {
    ...
    REGISTER_MJ_CONTROLLER(XLSController, "XLSController")
}