...
namespace FR3PCV
{
    ...
    : ControllerInterface(node, dt, std::move(jd))
    {
        ...
        key_sub_ = node_->create_subscription<std_msgs::msg::Int32>("fr3_pcv_controller/mode_input", 10,std::bind(&FR3PCVController::keyCallback, this, std::placeholders::_1));
        target_pose_sub_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>("fr3_pcv_controller/target_pose", 10,std::bind(&FR3PCVController::subtargetPoseCallback, this, std::placeholders::_1));
        base_vel_sub_ = node_->create_subscription<geometry_msgs::msg::Twist>("fr3_pcv_controller/cmd_vel", 10, std::bind(&FR3PCVController::subtargetBaseVelCallback, this, std::placeholders::_1));
        joint_sub_ = node_->create_subscription<sensor_msgs::msg::JointState>("/joint_states_raw", 10, std::bind(&FR3PCVController::subJointStatesCallback, this, std::placeholders::_1));

        ee_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("fr3_pcv_controller/ee_pose", 10);
    ...
    void FR3PCVController::starting()
    {
        ee_pose_pub_timer_ = node_->create_wall_timer(std::chrono::milliseconds(100), std::bind(&FR3PCVController::pubEEPoseCallback, this));
    }

    void FR3PCVController::updateState(const VecMap& pos_dict, 
                                         ...
                                         double current_time)
    {
        ...
        {
            RCLCPP_ERROR(node_->get_logger(), "[FR3PCVRobotData] Failed to update robot state.");
        }
    ...
    CtrlInputMap FR3PCVController::getCtrlInput() const
    {
    ...
    void FR3PCVController::setMode(const std::string& mode)
    {
    ...
    void FR3PCVController::keyCallback(const std_msgs::msg::Int32::SharedPtr msg)
    {
    ...
    void FR3PCVController::subtargetPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
    ...
    void FR3PCVController::subtargetBaseVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
    {
        RCLCPP_INFO(node_->get_logger(),
    ...
    void FR3PCVController::subJointStatesCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
    {
    ...
    void FR3PCVController::pubEEPoseCallback()
    {
    ...
    REGISTER_MJ_CONTROLLER(FR3PCVController, "FR3PCVController")
}