#include "fr3_husky/controller.h"

namespace FR3Husky
{
    FR3HuskyController::FR3HuskyController(const rclcpp::Node::SharedPtr& node, double dt, JointDict jd)
    : ControllerInterface(node, dt, std::move(jd))
    {
        robot_data_ = std::make_shared<FR3HuskyRobotData>();
        ...
        target_pose_sub_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>("fr3_husky_controller/target_pose", 10,std::bind(&FR3HuskyController::subtargetPoseCallback, this, std::placeholders::_1));
        base_vel_sub_ = node_->create_subscription<geometry_msgs::msg::Twist>("fr3_husky_controller/cmd_vel", 10, std::bind(&FR3HuskyController::subtargetBaseVelCallback, this, std::placeholders::_1));
        ...
        ee_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("fr3_husky_controller/ee_pose", 10);
    ...
    FR3HuskyController::~FR3HuskyController()
    {
    ...
    {
        ee_pose_pub_timer_ = node_->create_wall_timer(std::chrono::milliseconds(100), std::bind(&FR3HuskyController::pubEEPoseCallback, this));
    }

    void FR3HuskyController::updateState(const VecMap& pos_dict, 
                                         ...
                                         double current_time)
    {
        ...
        {
            RCLCPP_ERROR(node_->get_logger(), "[FR3HuskyRobotData] Failed to update robot state.");
        }
    ...
    CtrlInputMap FR3HuskyController::getCtrlInput() const
    {
    ...
    void FR3HuskyController::subtargetPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
    ...
    REGISTER_MJ_CONTROLLER(FR3HuskyController, "FR3HuskyController")
}