...
namespace PCV
{
    PCVController::PCVController(const rclcpp::Node::SharedPtr& node, double dt, JointDict jd)
    : ControllerInterface(node, dt, std::move(jd))
    {
        robot_data_ = std::make_shared<PCVRobotData>();
        robot_controller_ = std::make_unique<RobotController::Mobile::MobileBase>(dt_, robot_data_);

        key_sub_ = node_->create_subscription<std_msgs::msg::Int32>("pcv_controller/mode_input", 10,std::bind(&PCVController::keyCallback, this, std::placeholders::_1));
        target_pose_sub_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>("pcv_controller/target_pose", 10,std::bind(&PCVController::subtargetPoseCallback, this, std::placeholders::_1));
        target_vel_sub_ = node_->create_subscription<geometry_msgs::msg::Twist>("pcv_controller/cmd_vel", 10,std::bind(&PCVController::subtargetVelCallback, this, std::placeholders::_1));
        joint_sub_ = node_->create_subscription<sensor_msgs::msg::JointState>("/joint_states_raw", 10, std::bind(&PCVController::subJointStatesCallback, this, std::placeholders::_1));
        
        base_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("pcv_controller/base_pose", 10);
        base_vel_pub_ = node_->create_publisher<geometry_msgs::msg::Twist>("pcv_controller/base_vel", 10);
    ...
    PCVController::~PCVController()
    {

    }

    void PCVController::starting()
    {
        base_pose_pub_timer_ = node_->create_wall_timer(std::chrono::milliseconds(100), std::bind(&PCVController::pubBasePoseCallback, this));
        base_vel_pub_timer_ = node_->create_wall_timer(std::chrono::milliseconds(100), std::bind(&PCVController::pubBaseVelCallback, this));
    }

    void PCVController::updateState(const VecMap& pos_dict, 
                                      ...
                                      double current_time)
    {
        ...
        wheel_pos_(0) = pos_dict.at("front_left_steer")(0);
        wheel_pos_(1) = pos_dict.at("front_left_rotate")(0);
        wheel_pos_(2) = pos_dict.at("front_right_steer")(0);
        wheel_pos_(3) = pos_dict.at("front_right_rotate")(0);
        wheel_pos_(4) = pos_dict.at("rear_left_steer")(0);
        wheel_pos_(5) = pos_dict.at("rear_left_rotate")(0);
        wheel_pos_(6) = pos_dict.at("rear_right_steer")(0);
        wheel_pos_(7) = pos_dict.at("rear_right_rotate")(0);

        wheel_vel_(0) = vel_dict.at("front_left_steer")(0);
        wheel_vel_(1) = vel_dict.at("front_left_rotate")(0);
        wheel_vel_(2) = vel_dict.at("front_right_steer")(0);
        wheel_vel_(3) = vel_dict.at("front_right_rotate")(0);
        wheel_vel_(4) = vel_dict.at("rear_left_steer")(0);
        wheel_vel_(5) = vel_dict.at("rear_left_rotate")(0);
        wheel_vel_(6) = vel_dict.at("rear_right_steer")(0);
        wheel_vel_(7) = vel_dict.at("rear_right_rotate")(0);
    ...
    CtrlInputMap PCVController::getCtrlInput() const
    {
        CtrlInputMap ctrl_dict;
        ctrl_dict["front_left_steer"]   = wheel_vel_desired_(0);
        ctrl_dict["front_left_rotate"]  = wheel_vel_desired_(1);
        ctrl_dict["front_right_steer"]  = wheel_vel_desired_(2);
        ctrl_dict["front_right_rotate"] = wheel_vel_desired_(3);
        ctrl_dict["rear_left_steer"]    = wheel_vel_desired_(4);
        ctrl_dict["rear_left_rotate"]   = wheel_vel_desired_(5);
        ctrl_dict["rear_right_steer"]   = wheel_vel_desired_(6);
        ctrl_dict["rear_right_rotate"]  = wheel_vel_desired_(7);
        
        return ctrl_dict;
    }

     void PCVController::setMode(const std::string& mode)
    {
    ...
    void PCVController::keyCallback(const std_msgs::msg::Int32::SharedPtr msg)
    {
    ...
    void PCVController::subtargetPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
    ...
    void PCVController::subtargetVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
    {
        RCLCPP_INFO(node_->get_logger(),
    ...
    void PCVController::subJointStatesCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
    {
    ...
    void PCVController::pubBasePoseCallback()
    {
    ...
    void PCVController::pubBaseVelCallback()
    {
    ...
    REGISTER_MJ_CONTROLLER(PCVController, "PCVController")
}