...
from dyros_robot_menagerie.fr3_husky.robot_data import (FR3HuskyRobotData,
                                                        TASK_DOF,
                                                        VIRTUAL_DOF,
                                                        MANI_DOF,
                                                        MOBI_DOF,
...
class FR3HuskyControllerPy(ControllerInterface):
    ...
    def __init__(self, node: Node, dt: float, mj_joint_dict: Dict[str, Any]) -> None:
        ...
        self._joint_pub = node.create_publisher(JointState, "/joint_states_f", 10)

        self._azure_rgb_pub = node.create_publisher(Image, f"{ns}/kinect/image_raw", 10)
    ...
    def updateState(self,
        ...
        if not self.robot_data.update_state(self.q_virtual,    self.q_mobile,    self.q_mani,
            ...
            self.node.get_logger().error("[FR3HuskyRobotData] Failed to update robot state.")

        # get ee
        self.x     = self.robot_data.get_pose()
        self.xdot  = self.robot_data.get_velocity()

        for cam in ("azure_rgb", "d435_rgb"):
    ...
    def compute(self) -> None:
        ...
        if self.mode == "HOME":
            ...
            if self.mode == "QPIK":
                ...
                self.q_mani_desired = self.q_mani + self.dt * self.qdot_mani_desired
                ...
                qddot_mobile_desired, self.torque_mani_desired = self.robot_controller.QPID_cubic(x_target     = self.x_goal, 
                                                                                                  xdot_target  = np.zeros(TASK_DOF, dtype=float),
                                                                                                  x_init       = self.x_init, 
                                                                                                  xdot_init    = self.xdot_init,
                                                                                                  current_time = self.current_time, 
                                                                                                  init_time    = self.control_start_time, 
                ...
                self.qdot_mobile_desired = self.qdot_mobile_desired + self.dt * qddot_mobile_desired
    ...
    def getCtrlInput(self) -> Dict[str, float]:
        out = {"left_wheel":  self.qdot_mobile_desired[0],
               "right_wheel": self.qdot_mobile_desired[1]}
        for i in range(MANI_DOF):
    ...
    def _pub_image_cb(self) -> None:
        # kinect camera ---------------------------------------------------
        if self._latest_img["azure_rgb"] is not None:
            msg                  = self._bridge.cv2_to_imgmsg(self._latest_img["azure_rgb"], encoding="rgb8")
            msg.header.stamp     = self.node.get_clock().now().to_msg()
            msg.header.frame_id  = "base_link"
            self._azure_rgb_pub.publish(msg)
